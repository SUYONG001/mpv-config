-- ==============================================================================
-- mpv-osc-modern 原作者: maoiscat (email:valarmor@163.com)
-- https://github.com/maoiscat/mpv-osc-modern
--
-- 修改版 ModernX 作者: cyl0
-- https://github.com/cyl0/ModernX/
--
-- [2026.07.04] 全量中文注释解析版
-- ==============================================================================

-- 引入 mpv 底层的 API 模块，用于绘制图形、发送消息、读取配置等
...
-- ==============================================================================
-- 1. 用户配置区 (Parameters)
-- 这里的默认值可以通过外部的 osc.conf 文件进行覆盖修改
-- ==============================================================================
local user_opts = {
    showwindowed = true,        -- 窗口化时是否显示控制栏 (OSC)
    showfullscreen = true,      -- 全屏时是否显示 OSC
    idlescreen = true,          -- 空闲（未播放任何文件）时是否在屏幕中央绘制 mpv Logo 和提示文字
    scalewindowed = 1.0,        -- 窗口化时 OSC 界面的缩放比例
    scalefullscreen = 1.0,      -- 全屏时 OSC 界面的缩放比例
    scaleforcedwindow = 2.0,    -- 强制渲染窗口时的缩放比例
    vidscale = true,            -- OSC 是否跟随视频分辨率一起缩放
    hidetimeout = 1500,         -- 鼠标静止多少毫秒后自动隐藏 OSC（负数代表永远显示）
    fadeduration = 250,         -- OSC 隐藏/显示时的淡入淡出动画时长（毫秒，0代表无动画）
    minmousemove = 1,           -- 鼠标至少移动多少像素才会唤醒隐藏的 OSC
    iamaprogrammer = false,     -- 是否禁用 OSC 内部的轨道列表管理逻辑（极客选项）
    font = 'mpv-osd-symbols',   -- OSC 使用的默认字体
    seekbarhandlesize = 1.0,    -- 进度条上的拖动滑块的大小比例，范围 0 ~ 1
    seekrange = true,           -- 是否在进度条上显示缓存进度（灰色阴影部分）
    seekrangealpha = 64,        -- 缓存进度的透明度
    seekbarkeyframes = true,    -- 拖拽进度条时是否按照关键帧跳转（开启更流畅，关闭更精准但容易卡）
    showjump = true,            -- 是否显示“快进/快退”按钮（默认按住Shift左键单帧步进，右键跳1分钟）
    jumpamount = 5,             -- 点击快进/快退按钮时的默认跳转秒数
    jumpiconnumber = true,      -- 当跳转秒数为 5, 10, 30 时，是否在按钮图标上显示对应数字
    jumpmode = 'exact',         -- 跳转模式，'exact'为精准跳转，'relative+keyframes'为关键帧跳转
    title = '${media-title}',   -- 顶部标题栏显示的变量逻辑（默认显示媒体标题）
    showtitle = true,           -- 是否在顶部显示标题
    showonpause = true,         -- 暂停时是否强制保持 OSC 显示（不自动隐藏）
    timetotal = true,           -- 右侧时间是显示“总时长”还是“剩余时间”
    timems = false,             -- 是否精确显示时间到毫秒
    visibility = 'auto',        -- 启动时的默认可见性模式
    windowcontrols = 'auto',    -- 是否在右上角显示窗口控制按钮（最小化/最大化/关闭）
    greenandgrumpy = false,     -- 是否在 12 月份禁用 mpv Logo 上的彩蛋（圣诞帽）
    language = 'eng',           -- 界面语言：eng=英文，chs=中文，pl=波兰语
    volumecontrol = true,       -- 是否显示静音按钮和音量调节滑块
    keyboardnavigation = false, -- 是否允许使用键盘方向键在 OSC 按钮之间进行焦点导航
    chapter_fmt = "Chapter: %s",-- 鼠标悬停在进度条章节节点时显示的文本格式，"no"为禁用
    seekbarfgcolor = 'FF3399',  -- 新增：进度条高亮颜色 (采用 BGR 十六进制格式)
}

-- 定义不同秒数对应的快进/快退图标（使用特定的字体编码绘制）
local jumpicons = { 
...
-- 定义底部各种功能按钮的图标（使用 Material Design 图标字体）
local icons = {
  previous = '\239\142\181',    -- 上一集
  next = '\239\142\180',        -- 下一集
  play = '\239\142\170',        -- 播放
  pause = '\239\142\167',       -- 暂停
  backward = '\239\142\160',    -- 后退
  forward = '\239\142\159',     -- 前进
  audio = '\239\142\183',       -- 音频轨道
  volume = '\239\142\188',      -- 音量
  volume_mute = '\239\142\187', -- 静音
  sub = '\239\143\147',         -- 字幕轨道
  minimize = '\239\133\172',    -- 最小化
  fullscreen = '\239\133\173',  -- 全屏
  info = '',                   -- 性能信息 (OSD 统计)
}
...
local language = {
    ['eng'] = { -- 英文提示词
        ...
        off = 'OFF', na = 'n/a', none = 'none', video = 'Video', audio = 'Audio',
        subtitle = 'Subtitle', available = 'Available ', track = ' Tracks:',
        playlist = 'Playlist', nolist = 'Empty playlist.', chapter = 'Chapter', nochapter = 'No chapters.',
    },
    ['chs'] = { -- 中文提示词
        welcome = '{\\1c&H00\\bord0\\fs30\\fn微软雅黑 light\\fscx125}MPV{\\fscx100} 播放器',
        off = '关闭', na = 'n/a', none = '无', video = '视频', audio = '音频',
        subtitle = '字幕', available = '可选', track = '：',
        playlist = '播放列表', nolist = '无列表信息', chapter = '章节', nochapter = '无章节信息',
    },
    ['pl'] = { -- 波兰语提示词
        welcome = '{\\fs24\\1c&H0&\\1c&HFFFFFF&}Upuść plik lub łącze URL do odtworzenia.',
        off = 'WYŁ.', na = 'n/a', none = 'nic', video = 'Wideo', audio = 'Ścieżka audio',
        subtitle = 'Napisy', available = 'Dostępne ', track = ' Ścieżki:',
        playlist = 'Lista odtwarzania', nolist = 'Lista odtwarzania pusta.', chapter = 'Rozdział', nochapter = 'Brak rozdziałów.',
    }
}
...
-- OSC 内部计算使用的一些画布参数
local osc_param = {
    playresy = 0, playresx = 0, display_aspect = 1, unscaled_y = 0, areas = {},
}

-- ==============================================================================
-- 2. 界面样式定义区 (OSC Styles)
-- 使用 ASS 特效字幕的标签语法来控制模糊、边框、颜色、字体等外观
-- ==============================================================================
local osc_styles = {
    TransBg = '{\\blur100\\bord150\\1c&H000000&\\3c&H000000&}',     -- 底部控制栏的黑色渐变背景
    SeekbarBg = '{\\blur0\\bord0\\1c&HFFFFFF&}',                    -- 进度条的底色（白色）
    SeekbarFg = '{\\blur1\\bord1\\1c&H' .. user_opts.seekbarfgcolor .. '&}', -- 进度条的高亮前景色（动态读取用户配置）
    VolumebarBg = '{\\blur0\\bord0\\1c&H999999&}',                  -- 音量条的底色（灰色）
    VolumebarFg = '{\\blur1\\bord1\\1c&HFFFFFF&}',                  -- 音量条的高亮前景色（白色）
    Ctrl1 = '{\\blur0\\bord0\\1c&HFFFFFF&\\3c&HFFFFFF&\\fs36\\fnmaterial-design-iconic-font}', -- 播放/暂停大按钮样式
    Ctrl2 = '{\\blur0\\bord0\\1c&HFFFFFF&\\3c&HFFFFFF&\\fs24\\fnmaterial-design-iconic-font}', -- 其它中型按钮样式
    Ctrl2Flip = '{\\blur0\\bord0\\1c&HFFFFFF&\\3c&HFFFFFF&\\fs24\\fnmaterial-design-iconic-font\\fry180}', -- 图标水平翻转
    Ctrl3 = '{\\blur0\\bord0\\1c&HFFFFFF&\\3c&HFFFFFF&\\fs24\\fnmaterial-design-iconic-font}', -- 轨道切换等小型按钮样式
    Time = '{\\blur0\\bord0\\1c&HFFFFFF&\\3c&H000000&\\fs17\\fn' .. user_opts.font .. '}',     -- 时间戳文本样式
    Tooltip = '{\\blur1\\bord0.5\\1c&HFFFFFF&\\3c&H000000&\\fs18\\fn' .. user_opts.font .. '}',-- 鼠标悬停时的工具提示样式
    Title = '{\\blur1\\bord0.5\\1c&HFFFFFF&\\3c&H0\\fs38\\q2\\fn' .. user_opts.font .. '}',    -- 顶部标题文本样式
    WinCtrl = '{\\blur1\\bord0.5\\1c&HFFFFFF&\\3c&H0\\fs20\\fnmpv-osd-symbols}',               -- 右上角窗口控制按钮样式
    elementDown = '{\\1c&H999999&}',                                -- 鼠标按下时的按钮变暗反馈颜色
    elementHighlight = '{\\blur1\\bord1\\1c&HFFC033&}',             -- 键盘导航时的焦点高亮颜色
}

-- ==============================================================================
-- 3. 内部状态机 (Internal States) - 绝对不要手动修改这里面的初始值
-- 用于记录当前播放器是不是全屏、鼠标在不在窗口里、上次点击的时间等
-- ==============================================================================
local state = {
    showtime = nil,                         -- 上次唤醒 OSC 的时间戳
    osc_visible = false,                    -- 当前 OSC 是否可见
    anistart = nil, anitype = nil, animation = nil, -- 动画计算相关的状态
    mouse_down_counter = 0,                 -- 记录鼠标按下的时长（用于快进/快退长按连发）
    active_element = nil,                   -- 当前正在被操作（按下）的 UI 元素
    active_event_source = nil,              -- 触发事件的输入源
    rightTC_trem = not user_opts.timetotal, -- 右侧时间文本框是显示总时间还是剩余时间
    mp_screen_sizeX = nil, mp_screen_sizeY = nil, -- 上一次的屏幕分辨率（用于检测窗口大小改变）
    initREQ = false,                        -- 是否需要重新初始化 UI（比如全屏切换时）
    last_mouseX = nil, last_mouseY = nil,   -- 鼠标上次的坐标（用于判断是否发生了真实移动）
    mouse_in_window = false,                -- 鼠标是否在播放器窗口范围内
    message_text = nil, message_hide_timer = nil, -- 屏幕中央消息文本及隐藏定时器
    fullscreen = false,                     -- 是否处于全屏状态
    tick_timer = nil, tick_last_time = 0,   -- 逐帧渲染 (tick) 定时器记录
    hide_timer = nil, cache_state = nil,    -- 隐藏定时器及缓存状态
    idle = false, enabled = true,           -- 是否空闲、OSC功能是否启用
    input_enabled = true, showhide_enabled = false, 
    dmx_cache = 0, border = true, maximized = false,
    osd = mp.create_osd_overlay('ass-events'), -- 核心：创建一个用于渲染一切 UI 的 ASS 图层
    mute = false, lastvisibility = user_opts.visibility,
    fulltime = user_opts.timems, highlight_element = 'cy_audio',
    chapter_list = {},                      -- 按时间排序的章节列表
}
...
local is_december = os.date("*t").month == 12 -- 判断是否是 12 月（决定要不要画圣诞帽彩蛋）

-- 如果启用了 mpv 自带的丑陋默认 OSC，强制将其关闭，交由本脚本全面接管
...
function build_keyboard_controls()
    ...
    if user_opts.showjump then table.insert(bottom_button_line, 'jumpback') end
    table.insert(bottom_button_line, 'playpause')
    if user_opts.showjump then table.insert(bottom_button_line, 'jumpfrwd') end
...
-- ==============================================================================
-- 4. 数学与坐标辅助函数 (Helperfunctions)
-- 处理虚拟坐标、实际分辨率转换、计算碰撞箱(hitbox)等底层逻辑
-- ==============================================================================

-- 提交最终组合好的 ASS 特效文本给屏幕渲染引擎
function set_osd(res_x, res_y, text)
    if state.osd.res_x == res_x and state.osd.res_y == res_y and state.osd.data == text then
        return -- 如果内容和分辨率没变，跳过渲染以节省 CPU
    end
    state.osd.res_x = res_x
    state.osd.res_y = res_y
    state.osd.data = text
    state.osd.z = 1000 -- 确保 UI 永远盖在画面最顶层
    state.osd:update()
end
...
-- 数学映射：将一个范围内的值，按比例映射到另一个范围（用于计算进度条坐标）
...
-- 根据控件坐标(x,y)、宽高(w,h)和对齐方式(an，类似小键盘1-9)，计算控件四个角的碰撞边界
...
function limit_range(min, max, val)
    if val > max then val = max elseif val < min then val = min end
    return val
end

-- 进度条专用：通过播放百分比，反推算出进度条滑块应该在屏幕上的 X 轴位置
function get_slider_ele_pos_for(element, val)
    local ele_pos = scale_value(
        element.slider.min.value, element.slider.max.value,
        element.slider.min.ele_pos, element.slider.max.ele_pos, val)
    return limit_range(element.slider.min.ele_pos, element.slider.max.ele_pos, ele_pos)
end

-- 进度条专用：通过鼠标在屏幕上的 X 轴位置，反推算出该处代表的播放百分比
...
-- 在生成的 ASS 绘图指令里插入混合后的透明度标签
function ass_append_alpha(ass, alpha, modifier)
    local ar = {}
    for ai, av in pairs(alpha) do
        av = mult_alpha(av, modifier)
        if state.animation then av = mult_alpha(av, state.animation) end
        ar[ai] = av
    end
...
function ass_draw_rr_h_cw(ass, x0, y0, x1, y1, r1, hexagon, r2)
    if hexagon then ass:hexagon_cw(x0, y0, x1, y1, r1, r2)
    else ass:round_rect_cw(x0, y0, x1, y1, r1, r2) end
end
function ass_draw_rr_h_ccw(ass, x0, y0, x1, y1, r1, hexagon, r2)
    if hexagon then ass:hexagon_ccw(x0, y0, x1, y1, r1, r2)
    else ass:round_rect_ccw(x0, y0, x1, y1, r1, r2) end
end


-- ==============================================================================
-- 5. 轨道列表管理 (Tracklist Management)
-- 用于获取和切换当前视频的音轨、字幕轨
-- ==============================================================================
local nicetypes = {video = texts.video, audio = texts.audio, sub = texts.subtitle}

-- 解析原生轨道数据，将其整理为供 OSC 面板使用的干净表格
...
-- ==============================================================================
-- 6. 元素创建与预渲染 (Element Management)
-- 为所有的按钮、进度条赋予坐标、宽高、碰撞箱，并生成它们静态部分的绘图代码
-- ==============================================================================
local elements = {}
...
function prepare_elements()
    ...
    -- 按照图层(layer)重新排序，确保靠后的图层画在上面
    function elem_compare (a, b) return a.layout.layer < b.layout.layer end
    ...
    for _,element in pairs(elements) do
        ...
        -- 针对进度条（slider），预先画出它固定不变的部分（例如底槽和章节标记点）
                ...
                for _,marker in pairs(markers) do
                    if (marker >= element.slider.min.value) and (marker <= element.slider.max.value) then
                        ...
                        if (slider_lo.gap > 5) then 
                            ...
                            if (slider_lo.nibbles_top) then static_ass:rect_cw(s - 1, 0, s + 1, slider_lo.gap) end
                            if (slider_lo.nibbles_bottom) then static_ass:rect_cw(s - 1, elem_geo.h-slider_lo.gap, s + 1, elem_geo.h) end
                        end
                    end
                end
            end
...
-- ==============================================================================
-- 7. 动态渲染引擎 (Element Rendering)
-- 每一帧都会调用这里，去刷新进度条走势、更新文本，并处理鼠标悬停的缩略图弹出
-- ==============================================================================
...
function render_elements(master_ass)
    -- 计算标题栏内容（如果是悬停在章节节点，强制将顶部标题变更为该章节的名字）
        ...
        if (element.type == 'slider') then
            ...
            if not (element.slider.tooltipF == nil) then
                if mouse_hit(element) then
                    ...
                    local ty
                    if (an == 2) then ty = element.hitbox.y1 else ty = element.hitbox.y1 + elem_geo.h/2 end

                    local tx = get_virt_mouse_pos()
                    -- 防溢出逻辑：当鼠标靠在最左边/最右边时，提示框会偏离以防止超出屏幕
                    ...
                    -- 最核心：在这里向 Thumbfast 发出“生成某时间点缩略图”的呼叫指令
                    ...
                    -- 鼠标移出进度条区域，发送指令要求 thumbfast 销毁图像
        ...
        elseif (element.type == 'button') then
            ...
            if not (element.tooltipF == nil) and element.enabled then
                if mouse_hit(element) then
                    ...
                    if type(element.tooltipF) == 'function' then tooltiplabel = element.tooltipF() else tooltiplabel = element.tooltipF end
                    elem_ass:new_event()
                    elem_ass:pos(tx, ty)
                    elem_ass:an(an)
...
-- ==============================================================================
-- 8. 屏幕中央文字消息逻辑 (Message display)
-- 当你按下右键看播放列表，或者按下 Shift 拖动进度条时，屏幕中央呈现的那些字
-- ==============================================================================
...
function get_chapterlist()
    ...
    for i, v in ipairs(limlist) do
        ...
        local title = v.title
        if title == nil then title = string.format(texts.chapter .. ' %02d', i) end
        message = string.format('%s[%s] %s %s\n', message, time, (v.current and '●' or '○'), title)
    end
    return message
end

-- 接收发送指令并记录要显示多久（时长定时器）
function show_message(text, duration)
    if duration == nil then
        duration = tonumber(mp.get_property('options/osd-duration')) / 1000
    elseif not type(duration) == 'number' then print('duration: ' .. duration) end

    text = string.sub(text, 0, 4000)
    text = string.gsub(text, '\n', '\\N')
    state.message_text = text

    if not state.message_hide_timer then state.message_hide_timer = mp.add_timeout(0, request_tick) end
...
-- ==============================================================================
-- 9. 界面构建初始化 (Initialisation and Layout)
-- 根据屏幕尺寸，从无到有地建立起一套控件坐标，给按键绑定点击事件
-- ==============================================================================
...
function new_element(name, type)
    ...
    if (type == 'slider') then elements[name].slider = {min = {value = 0}, max = {value = 100}} end
    ...
    if not (elements[name] == nil) then
        ...
        elseif (elements[name].type == 'slider') then
            elements[name].layout.slider = {
                border = 1, gap = 1, nibbles_top = true, nibbles_bottom = true,
                adjust_tooltip = true, tooltip_style = '', tooltip_an = 2,
...
function window_controls()
    ...
    lo = add_layout('close'); lo.geometry = third_geo; lo.style = osc_styles.WinCtrl; lo.alpha[3] = 0

    -- 最小化按钮
    ne = new_element('minimize', 'button')
    ne.content = '\\n\238\132\146'
    ne.eventresponder['mbtn_left_up'] = function () mp.commandv('cycle', 'window-minimized') end
    lo = add_layout('minimize'); lo.geometry = first_geo; lo.style = osc_styles.WinCtrl; lo.alpha[3] = 0
    
    -- 最大化/还原按钮
    ne = new_element('maximize', 'button')
    if state.maximized or state.fullscreen then ne.content = '\238\132\148' else ne.content = '\238\132\147' end
    ne.eventresponder['mbtn_left_up'] = function ()
        if state.fullscreen then mp.commandv('cycle', 'fullscreen')
        else mp.commandv('cycle', 'window-maximized') end
    end
    lo = add_layout('maximize'); lo.geometry = second_geo; lo.style = osc_styles.WinCtrl; lo.alpha[3] = 0
end

-- 这就是最硬核的“装潢图纸”：决定每个按钮、滑块在你屏幕下方的 XY 坐标
...
-- ==============================================================================
-- 10. OSC 核心初始化逻辑 (osc_init)
-- 这里是将按钮“赋予灵魂”的地方，告诉按键被点击后要执行什么命令
-- ==============================================================================
function osc_init()
    msg.debug('osc_init')
    ...
    if (mp.get_property('video') == 'no') then scale = user_opts.scaleforcedwindow
    elseif state.fullscreen then scale = user_opts.scalefullscreen
    else scale = user_opts.scalewindowed end

    if user_opts.vidscale then osc_param.unscaled_y = baseResY
    else osc_param.unscaled_y = display_h end
    ...
    ne.content = function ()
        if mp.get_property('pause') == 'yes' then return (icons.play) else return (icons.pause) end
    end
    ne.eventresponder['mbtn_left_up'] = function ()
    ...
    if user_opts.showjump then
        ...
        if user_opts.jumpiconnumber then icons = jumpicons[jumpamount] or jumpicons.default end
    ...
    ne.eventresponder['shift+mbtn_left_down'] = function () show_message(get_tracklist('audio')) end
    ...
    ne.content = function ()
        if (state.fullscreen) then return (icons.minimize) else return (icons.fullscreen) end
    end
    ne.visible = (osc_param.playresx >= 540)
    ...
    ne.content = function ()
        ...
        if state.paused then title = title:gsub('\\n', ' '):gsub('\\$', ''):gsub('{','\\{')
        else title = title:gsub('\\n', ' '):gsub('\\$', ''):gsub('{','\\{') end
    ...
    ne.content = function ()
        if (state.fulltime) then return (mp.get_property_osd('playback-time/full'))
    ...
    ne.content = function ()
        ...
        if (state.rightTC_trem) then
            if (state.fulltime) then return ('-'..mp.get_property_osd('playtime-remaining/full'))
            else return ('-'..mp.get_property_osd('playtime-remaining')) end
        else
            if (state.fulltime) then return (mp.get_property_osd('duration/full'))
...
-- ==============================================================================
-- 11. 其它底层控制逻辑 (Other important stuff)
-- 包括触发渲染动作、淡入淡出动画、隐藏计时器重置等
-- ==============================================================================
...
function show_osc()
    ...
    osc_visible(true)
    if user_opts.keyboardnavigation == true then osc_enable_key_bindings() end
...
function hide_osc()
    ...
    if not state.enabled then
        ...
        render_wipe()
        if user_opts.keyboardnavigation == true then osc_disable_key_bindings() end
...
-- 申请调用渲染（会根据刷新率限制调用频率以省电）
function request_tick()
    if state.tick_timer == nil then state.tick_timer = mp.add_timeout(0, tick) end
...
-- ==============================================================================
-- 12. 事件分发器 (Eventhandling)
-- 把鼠标移动、左键/右键按下、松开，转译成对应的操作传给按钮
-- ==============================================================================
...
function process_event(source, what)
    ...
    if what == 'down' or what == 'press' then
        for n = 1, #elements do
            if mouse_hit(elements[n]) and elements[n].eventresponder and
                ...
                if element_has_action(elements[n], action) then elements[n].eventresponder[action](elements[n]) end
            end
        end
    ...
    elseif what == 'up' then
        if elements[state.active_element] then
            ...
            if element_has_action(elements[n], 'reset') then elements[n].eventresponder['reset'](elements[n]) end
        end
        state.active_element = nil
...
-- ==============================================================================
-- 13. 空闲屏幕矢量绘图 (Logo & Santa hat)
-- 当没有播放视频时，在画面中央画出那个带有各种紫色块拼接的原生 mpv Logo
-- ==============================================================================
local logo_lines = {
    -- 白色底板边框
...
-- ==============================================================================
-- 14. 引擎核心驱动 (Tick & Observers)
-- mpv 会每一帧高速调用 tick，通过监听器将其绑定到播放器的各个行为上
-- ==============================================================================
function tick()
    if (not state.enabled) then return end

    if (state.idle) then
        -- 当闲置（也就是你刚打开 mpv 还没有拖入视频）时，执行这段画 Logo 的代码
        ...
        if user_opts.idlescreen then
            for i, line in ipairs(logo_lines) do
                ass:new_event(); ass:append(line_prefix .. line)
            end
        end
...
function enable_osc(enable)
    ...
    if enable then do_enable_keybindings()
    else
        hide_osc() 
        if state.showhide_enabled then
            mp.disable_key_bindings('showhide')
...
function update_duration_watch()
    local want_watch = user_opts.livemarkers and (mp.get_property_number("chapters", 0) or 0) > 0 and true or false
...
-- 将上述的一大堆逻辑挂载到 mpv 的系统事件中，确保它们能被精准触发
...
mp.register_script_message('osc-tracklist', function(dur)
    local msg = {}
    for k,v in pairs(nicetypes) do table.insert(msg, get_tracklist(k)) end
    show_message(table.concat(msg, '\n\n'), dur)
end)

-- 监听各种状态更改的事件（全屏切换、静音切换、窗口改变大小等）
mp.observe_property('fullscreen', 'bool', function(name, val) state.fullscreen = val; request_init_resize() end)
mp.observe_property('mute', 'bool', function(name, val) state.mute = val end)
mp.observe_property('border', 'bool', function(name, val) state.border = val; request_init_resize() end)
mp.observe_property('window-maximized', 'bool', function(name, val) state.maximized = val; request_init_resize() end)
mp.observe_property('idle-active', 'bool', function(name, val) state.idle = val; request_tick() end)
...
function visibility_mode(mode, no_osd)
    ...
    if not no_osd and tonumber(mp.get_property('osd-level')) >= 1 then mp.osd_message('OSC visibility: ' .. mode) end
...
-- ==============================================================================
-- 15. 键盘导航逻辑 (KeyboardControl)
-- 允许你使用键盘方向键来选择 UI 上的不同按钮并模拟按下回车来点击
-- ==============================================================================
...
function osc_kb_control_left()
    ...
    for row_name, row_controls in pairs(keyboard_controls) do
        ...
        if active_control_name == 'seekbar' then mp.commandv('seek', -5, 'exact', 'keyframes'); return end
...
-- ==============================================================================
-- 16. 缩略图信使监听器 (Thumbfast Listener)
-- 这里专门用来接听 thumbfast 脚本发来的“生成完毕”或状态消息
-- ==============================================================================
visibility_mode(user_opts.visibility, true)
