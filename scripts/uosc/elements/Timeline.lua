-- ==============================================================================
-- Timeline.lua — 时间轴/进度条组件
-- ==============================================================================
-- 功能概述：
--   1. 显示视频播放进度条，支持拖拽跳转、滚轮微调
--   2. 支持两种样式：'line'（细线）和 'bar'（粗条）
--   3. 支持精简进度条（progress）和全尺寸进度条的切换
--   4. 显示章节标记点（菱形图标）和 AB 循环点
--   5. 支持 YouTube 热力图（heatmap）叠加显示
--   6. 支持缩略图预览（通过 thumbfast 插件）
--   7. 显示缓存范围（流媒体缓冲指示）
--   8. 自定义章节范围标记（如 OP/ED 等）
--   9. 鼠标悬停时显示时间戳和章节标题
-- ==============================================================================
-- 交互方式：
--   - 点击进度条任意位置：跳转到该时间点（暂停播放）
--   - 拖拽进度条：实时跳转，松手后恢复播放状态
--   - 滚轮在进度条上滚动：按 config.timeline_step 步进跳转
--   - 悬停进度条：显示时间戳 + 缩略图（如果启用）
-- ==============================================================================
...
function Timeline:init()
    -- 调用父类 Element 的初始化
    -- render_order = 5 表示渲染在控制栏下方、幕布上方
    Element.init(self, 'timeline', {render_order = 5})

    ---@type false|{pause: boolean, distance: number, last: {x: number, y: number}}
    -- 拖拽状态：false 表示未拖拽，否则记录拖拽开始时的暂停状态、距离和鼠标位置
    self.pressed = false

    -- 是否被遮挡（空间不足时禁用）
    self.obstructed = false

    -- 当前有效尺寸（受 visibility 影响）
    self.size = 0

    -- 精简进度条尺寸（由 options.progress 控制）
    self.progress_size = 0

    -- 最小精简进度条尺寸（用于 flash-progress 动画）
...
-- ==============================================================================
-- 获取有效尺寸 (get_effective_size)
-- 根据可见度和精简模式计算实际显示的进度条高度
-- ==============================================================================
...
-- ==============================================================================
-- 精简进度条尺寸决策 (decide_progress_size)
-- ==============================================================================

function Timeline:decide_progress_size()
    -- 根据 options.progress 配置决定是否显示精简进度条
...
-- ==============================================================================
-- 时间计算：根据鼠标 X 坐标获取对应的时间点 (get_time_at_x)
-- ==============================================================================

---@param x number 鼠标在屏幕上的 X 坐标
---@return number 对应的时间点（秒）
function Timeline:get_time_at_x(x)
    -- 细线样式下，线本身占用一定宽度，需要特殊处理
...
-- ==============================================================================
-- 根据鼠标位置执行跳转 (set_from_cursor)
-- ==============================================================================

---@param fast? boolean true=关键帧跳转（快），false=精确跳转（准）
...
function Timeline:render()
    ...
    if is_line then
        -- 细线样式：进度条是一条细线，但悬停时会根据 progress_size 渐变粗
    ...
    ass:draw_start()
    ass:rect_cw(bax, bay, fax, bby)                  -- 已播放部分（由前景覆盖）
    ass:rect_cw(fbx, bay, bbx, bby)                  -- 未播放部分
    ass:rect_cw(fax, bay, fbx, fay)                  -- 进度条上方
    ass:draw_stop()
    ...
    if text_opacity > 0 then
        ...
        if cache_duration and options.buffered_time_threshold > 0
            ...
            local min_x = bax + spacing + margin + time_width
            local max_x = bbx - spacing - margin - time_width_end
        ...
        -- 目标时间（右侧，根据 destination_time 配置）
