-- ==============================================================================
-- visualizer.lua — 音频频谱可视化生成器
-- ==============================================================================
-- 功能概述：
--   1. 通过 mpv 的 lavfi-complex 滤镜链，将音频数据实时转换为可视化图形
--   2. 支持多种可视化类型：频谱图（CQT）、柱状频谱、矢量示波器、波形图、音量表
--   3. 支持多种工作模式：无视频时启用、无封面时启用、始终启用、禁用
--   4. 支持按 C 键循环切换可视化类型
-- ==============================================================================
-- 原作者：mpv-player 社区
-- 完成全量中文注释解析
...
-- ==============================================================================
-- 1. 用户配置区 (Options)
-- 这些默认值可通过 script-opts/visualizer.conf 文件覆盖修改
-- ==============================================================================
local opts = {
    -- 工作模式：
    -- off           = 完全禁用可视化
    -- noalbumart    = 无专辑封面且无视频时才启用可视化
    -- novideo       = 无视频轨道时启用可视化（有视频时自动切换回视频）
    -- force         = 始终启用可视化（覆盖视频画面）
    mode = "novideo",

    -- 可视化类型（按 C 键循环切换）：
    -- off           = 关闭
    -- showcqt       = 恒定 Q 变换频谱图（连续频谱）
    -- avectorscope  = 矢量示波器（相位图）
    -- showspectrum  = 柱状频谱图
    -- showcqtbar    = 恒定 Q 变换柱状频谱（条形图）
    -- showwaves     = 波形图
    -- showvolume    = 音量表
    name = "showcqt",
    ...
    -- medium  = 1280p @ 60fps
    -- high    = 1920p @ 60fps
    -- veryhigh = 2560p @ 60fps
    quality = "medium",

    -- 频谱高度系数（范围 4 ~ 12，值越大频谱占画面比例越高）
    height = 6,

    -- 是否强制运行可视化（即使 mpv 的 force-window 选项为 no）
    -- true  = 始终运行
    -- false = 仅在 force-window=yes 时运行
    forcewindow = true,
}

-- ==============================================================================
-- 2. 快捷键定义
-- 按 C 键可在所有可视化类型之间循环切换
-- ==============================================================================
local cycle_key = "c"

-- ==============================================================================
-- 3. 启动前检查：如果已经存在 lavfi-complex 滤镜则退出
-- 避免与其它滤镜脚本冲突
-- ==============================================================================
...
local visualizer_name_list = {
    ...
    "avectorscope",  -- 矢量示波器
    "showspectrum",  -- 柱状频谱
    "showcqtbar",    -- 恒定 Q 变换柱状图
    "showwaves",     -- 波形图
    "showvolume",    -- 音量表
}

-- ==============================================================================
-- 5. 坐标轴背景图片（Base64 编码的 PNG）
-- axis_0 用于 showcqt（连续频谱）的坐标轴刻度
-- axis_1 用于 showcqtbar（柱状频谱）的坐标轴刻度
-- 这些图片由 FFmpeg 的 showcqt 滤镜在渲染时叠加到画面上
-- ==============================================================================
...
"v4xehz+dX8yf1ov5q/Rn9fpHbQI8+vpHegLscucuTQnN7fnTvmK/5o7G85alpI2+AA3jaP5PwBt9eHfUa/kK8JsRNFOypmbxXRJ5DNetP7QltLsG+ArysPe2hi45z8hvP3EHuNzb1jLm7Y386/SHoX/1zJgfjU9M" ..
...
"QRAEQRAEQRAEQRAEQRAEQRAEQRAEQRBkq1gyK2F3Q1HCkpFHHnnkkUceeeSRRx555JFH/lHNu0KJiEqQRx555JFHHnnkkUceeeSRR/4RyzMIgiAIgiAIgiAImgTCAhiCIAiCIAiCIAiaFHK53JGoq4j8sa4i+lfkkUce" ..
...
"+mv4OuhPGbmB0bH/ZczsJGP7/E4fn939RWL92wFf103Rrebs9Tt9fHb3Z1Mk4g1goH2o6+SuH08wVnfo4fNTUZR36Wj9cca6W+qP15ZLn1ueML/Tx2dvf88v5NcsdDXVH68/7rnMmOS93RLmd/r47O7P5W6ubq6+" ..
...
-- ==============================================================================
-- 7. 核心函数：生成 FFmpeg 滤镜字符串 (get_visualizer)
-- 根据可视化类型、画质等级、视频轨道状态，生成对应的 lavfi-complex 滤镜链
-- 这是整个脚本最核心的部分，直接决定最终渲染效果
-- ==============================================================================
local function get_visualizer(name, quality, vtrack)
    local w, h, fps

    -- 根据画质等级设定分辨率与帧率
    -- 更高的分辨率/帧率会消耗更多 CPU/GPU 资源
    ...
    -- 根据 height 系数计算最终画面高度
    -- 值越大，频谱占画面比例越高
    h = w * opts.height / 16

    -- ================================================================
    -- 可视化类型：showcqt（恒定 Q 变换频谱）
    -- 这是最经典的频谱样式：连续渐变的热力图
    -- ================================================================
    ...
    -- ================================================================
    -- 可视化类型：avectorscope（矢量示波器）
    -- 显示音频的相位关系（立体声左右通道的差异）
    -- ================================================================
    ...
    -- ================================================================
    -- 可视化类型：showspectrum（柱状频谱）
    -- 传统的频率条状图，类似音乐播放器的均衡器
    -- ================================================================
    ...
    -- ================================================================
    -- 可视化类型：showcqtbar（恒定 Q 变换柱状图）
    -- showcqt 的条形图版本，每个频率以竖直条状呈现
    -- ================================================================
...
-- ==============================================================================
-- 9. 可视化钩子函数 (visualizer_hook)
-- 在文件加载时被调用，检测轨道状态并应用或移除可视化滤镜
-- ==============================================================================
local function visualizer_hook()
    ...
    -- 如果音视频轨道尚未被选中，从 track-list 中自动选取
    ...
    -- 生成对应的 lavfi-complex 滤镜字符串
    local lavfi = select_visualizer(vtrack, atrack)

    -- 防止无限循环：只有当滤镜内容发生变化时才更新
...
opts.height = math.min(12, math.max(4, opts.height))  -- 限制在 4~12 之间
opts.height = math.floor(opts.height)

-- 如果 forcewindow=false 且 mpv 的 force-window=no，则不启动
...
-- 11. 事件与属性监听
-- ==============================================================================
mp.add_hook("on_preloaded", 50, visualizer_hook)              -- 文件加载时触发
mp.observe_property("current-tracks/audio", "native", visualizer_hook)  -- 音轨变化时触发
mp.observe_property("current-tracks/video", "native", visualizer_hook)  -- 视频轨变化时触发

-- ==============================================================================
-- 12. 循环切换可视化类型 (cycle_visualizer)
-- 按 C 键时，在 visualizer_name_list 中向后循环切换
-- ==============================================================================
